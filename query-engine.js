...
// Fork of https://github.com/bevry/query-engine
...
(function(factory)
{
  ...
  var Criteria, Hash, Pill, Query, QueryCollection, queryEngine, util, _ref,
  ...
  util = {
    ...
    get: function(obj, key) {
      ...
      if (obj.get != null) {
        result = obj.get(key);
      } else if (typeof obj === 'object' && key.indexOf('.') !== -1) {
        keys = key.split('.', 2);
        if (keys.length === 1 && typeof obj[keys[0]] !== 'undefined') {
          result = obj[keys[0]];
        } else {
          result = this.get(obj[keys[0]], keys[1]);
        }
      } else {
        result = obj[key];
      }
